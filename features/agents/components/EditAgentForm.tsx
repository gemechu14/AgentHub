...
export function EditAgentForm({ agent }: EditAgentFormProps) {
  ...
  const [apiKey, setApiKey] = useState(agent.api_key || ""); // Use existing API key if available
  ...
  const [recommendedQuestions, setRecommendedQuestions] = useState<string[]>(
    agent.recommended_questions && agent.recommended_questions.length > 0
      ? agent.recommended_questions.slice(0, 2)
      : []
  );
  ...
  useEffect(() => {
    if (agent.connection_config) {
      if (agent.connection_type === "POWERBI") {
        ...
        // Set password from config if available (backend may not return it for security)
              ...
              {connectionType === "SQL Database" && (
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-xs text-slate-500">
                      {dbPassword ? "Password is set. Leave blank to keep current password, or enter a new password to update." : "Enter the database password. If updating, enter a new password."}
                    </p>
                  </div>
                </div>
              )}
